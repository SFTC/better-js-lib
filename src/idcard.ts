...
// 15位身份证转18位身份证
export function IDCard15To18(oldCard: string): string {
 ...
 */
function processIdCard(identityCard: string): string | boolean {
 ...
 * @param {String} identityCard 身份证号
 */
export function getBirthday(identityCard: string): { birthday: string; year: string; month: string; day: string } | -1 {
  // 校验过的身份证号
 ...
 */
export function getAge(identityCard: string, baseDate?: string): number {
  // 校验过的身份证号
  var proIdCard = processIdCard(identityCard);
 ...
 * @param {String} identityCard 身份证号
 */
export function getArea(identityCard: string): { province: string; city: string; district: string } | -1 {
  // 校验过的身份证号
  var proIdCard = processIdCard(identityCard);
 ...
 * @param {String} identityCard 身份证号
 */
export function getSex(identityCard: string): string | -1 {
  // 校验过的身份证号
  var proIdCard = processIdCard(identityCard);
 ...
 * @param {String} identityCard 身份证号
 */
export function getAnimalSigns(identityCard: string): string | -1 {
  // 校验过的身份证号
  var proIdCard = processIdCard(identityCard);
...
// 从身份证号中获取到相关的所有信息
export function idcard(identityCard: string): -1 | {
  birthday: {
    birthday: string;
    year: string;
    month: string;
  ...
  return {
    birthday: getBirthday(proIdCard as string),
    age: getAge(proIdCard as string),
    area: getArea(proIdCard as string),
    sex: getSex(proIdCard as string),
    animalSigns: getAnimalSigns(proIdCard as string),
  };
}
