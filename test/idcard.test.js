...
describe('解析身份证号', function () {
  ...
  describe('从身份证号中得到年龄', function () {
    ...
    it(`设置了基准时间，"131002199408153611" 应该返回"${age}"`, function () {
      expect(new Idcard('131002199408153611', 1554090947763).getAge()).to.equal(
        age
      );
    });
    it('"110102199901018781" 不是一个合法的身份证号', function () {
  ...
  describe('从身份证号中得到生肖', function () {
    ...
    while (++index < _length) {
      it(
        '"' + testIdCard[index] + '" 应该返回 "' + animalSigns[index] + '"',
        function (index) {
          expect(new Idcard(testIdCard[index]).getAnimalSigns()).to.equal(
            animalSigns[index]
          );
        }.bind(null, index)
      );
  ...
  describe('"15位"身份证号转换为"18位"', function () {
    it('"131002940815361" 应该返回 "131002199408153611"', function () {
      expect(new Idcard('131002940815361').IDCard15To18()).to.equal(
        '131002199408153611'
      );
    });
